...
RUN wget -O "$HOME/bin/hadolint" 'https://github.com/hadolint/hadolint/releases/download/v2.12.0/hadolint-Linux-x86_64' \
    && chmod +x "$HOME/bin/hadolint"

RUN TEMP_TAR_XZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_XZ" 'https://github.com/koalaman/shellcheck/releases/download/v0.9.0/shellcheck-v0.9.0.linux.x86_64.tar.xz' \
    && tar -xJf "$TEMP_TAR_XZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/shellcheck-v0.9.0/shellcheck" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_XZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/cantino/mcfly/releases/download/v0.8.4/mcfly-v0.8.4-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/sharkdp/bat/releases/download/v0.24.0/bat-v0.24.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/gcla/termshark/releases/download/v2.4.0/termshark_2.4.0_linux_x64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/BurntSushi/ripgrep/releases/download/14.0.3/ripgrep-14.0.3-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/sharkdp/fd/releases/download/v9.0.0/fd-v9.0.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/fd-v9.0.0-x86_64-unknown-linux-musl/fd" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/denisidoro/navi/releases/download/v2.23.0/navi-v2.23.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/lsd-rs/lsd/releases/download/v1.0.0/lsd-v1.0.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/sharkdp/hyperfine/releases/download/v1.18.0/hyperfine-v1.18.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/hyperfine-v1.18.0-x86_64-unknown-linux-musl/hyperfine" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/XAMPPRocky/tokei/releases/download/v13.0.0-alpha.0/tokei-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/ClementTsang/bottom/releases/download/0.9.6/bottom_x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/btm" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/orf/gping/releases/download/gping-v1.16.0/gping-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/orhun/kmon/releases/download/v1.6.4/kmon-1.6.4-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -O "$TEMP_TAR_GZ" 'https://github.com/jesseduffield/lazygit/releases/download/v0.40.2/lazygit_0.40.2_Linux_x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
