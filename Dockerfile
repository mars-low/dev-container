...
RUN export DEBIAN_FRONTEND=noninteractive \ 
    ...
    && wget -q -O "/etc/apt/sources.list.d/xpra.sources" https://xpra.org/repos/focal/xpra.sources \
    && wget -q -O "/etc/apt/sources.list.d/xpra-beta.sources" https://xpra.org/repos/focal/xpra-beta.sources \
    && apt-get clean -y && rm -rf /var/lib/apt/lists/* /tmp/library-scripts

# Add the Microsoft package repository (to install dotnet in the next step)
RUN TEMP_DEB="$(mktemp)" \
    && wget -q -O "$TEMP_DEB" 'https://packages.microsoft.com/config/ubuntu/20.04/packages-microsoft-prod.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"
...
RUN TEMP_DEB="$(mktemp)" \
    && wget -q -O "$TEMP_DEB" 'https://github.com/coder/code-server/releases/download/v4.20.0/code-server_4.20.0_amd64.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"

RUN TEMP_DEB="$(mktemp)" \
    && wget -q -O "$TEMP_DEB" 'https://github.com/PowerShell/PowerShell/releases/download/v7.4.0/powershell_7.4.0-1.deb_amd64.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"

RUN TEMP_DEB="$(mktemp)" \
    && wget -q -O "$TEMP_DEB" 'https://releases.hashicorp.com/vagrant/2.3.7/vagrant_2.3.7-1_amd64.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/tmux/tmux/releases/download/3.3a/tmux-3.3a.tar.gz' \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/jonas/tig/releases/download/tig-2.5.8/tig-2.5.8.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TBZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TBZ" 'https://github.com/aristocratos/btop/releases/download/v1.2.13/btop-x86_64-linux-musl.tbz' \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/gcla/termshark/releases/download/v2.4.0/termshark_2.4.0_linux_x64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/peco/peco/releases/download/v0.5.11/peco_linux_amd64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/peco_linux_amd64/peco" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://dev.yorhel.nl/download/ncdu-2.2.1-linux-x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/dandavison/delta/releases/download/0.16.5/delta-0.16.5-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/BurntSushi/ripgrep/releases/download/14.0.3/ripgrep-14.0.3-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/muesli/duf/releases/download/v0.8.1/duf_0.8.1_linux_x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_ZIP="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_ZIP" 'https://github.com/tstack/lnav/releases/download/v0.11.1/lnav-0.11.1-x86_64-linux-musl.zip' \
...
RUN TEMP_ZIP="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_ZIP" 'https://github.com/Canop/broot/releases/download/v1.30.2/broot_1.30.2.zip' \
    && unzip "$TEMP_ZIP" -d "$TEMP_DIR" \
    && mv "${TEMP_DIR}/x86_64-unknown-linux-musl/broot" "$HOME/bin" \
    && rm -rf "$TEMP_ZIP" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/eza-community/eza/releases/download/v0.17.0/eza_x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/starship/starship/releases/download/v1.17.0/starship-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/sharkdp/fd/releases/download/v9.0.0/fd-v9.0.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/fd-v9.0.0-x86_64-unknown-linux-musl/fd" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/denisidoro/navi/releases/download/v2.23.0/navi-v2.23.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/lsd-rs/lsd/releases/download/v1.0.0/lsd-v1.0.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/lsd-v1.0.0-x86_64-unknown-linux-musl/lsd" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/extrawurst/gitui/releases/download/v0.24.3/gitui-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/sharkdp/hyperfine/releases/download/v1.18.0/hyperfine-v1.18.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/hyperfine-v1.18.0-x86_64-unknown-linux-musl/hyperfine" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/XAMPPRocky/tokei/releases/download/v13.0.0-alpha.0/tokei-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/bootandy/dust/releases/download/v0.8.6/dust-v0.8.6-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/dust-v0.8.6-x86_64-unknown-linux-musl/dust" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/pemistahl/grex/releases/download/v1.4.4/grex-v1.4.4-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/ClementTsang/bottom/releases/download/0.9.6/bottom_x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/btm" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/orf/gping/releases/download/gping-v1.16.0/gping-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/orhun/kmon/releases/download/v1.6.4/kmon-1.6.4-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
    && mv "${TEMP_DIR}/kmon-1.6.4/kmon" "$HOME/bin" \
    && rm -rf "$TEMP_TAR_GZ" "$TEMP_DIR"

RUN TEMP_TAR_GZ="$(mktemp)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/zellij-org/zellij/releases/download/v0.39.2/zellij-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$HOME/bin" \
    && rm "$TEMP_TAR_GZ"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/chmln/sd/releases/download/v1.0.0/sd-v1.0.0-x86_64-unknown-linux-musl.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
# RUN TEMP_ZIP="$(mktemp)" \
#     TEMP_DIR="$(mktemp -d)" \
#     && wget -q -O "$TEMP_ZIP" 'https://github.com/MordechaiHadad/bob/releases/download/v2.7.0/bob-linux-x86_64.zip' \
#     && unzip "$TEMP_ZIP" -d "$TEMP_DIR" \
...
RUN wget -q -O "$HOME/bin/bazelisk" 'https://github.com/bazelbuild/bazelisk/releases/download/v1.19.0/bazelisk-linux-amd64' \
    && chmod +x "$HOME/bin/bazelisk"

RUN wget -q -O "$HOME/bin/bombardier" 'https://github.com/codesenberg/bombardier/releases/download/v1.2.6/bombardier-linux-amd64' \
    && chmod +x "$HOME/bin/bombardier"

RUN wget -q -O "$HOME/bin/fx" 'https://github.com/antonmedv/fx/releases/download/31.0.0/fx_linux_amd64' \
    && chmod +x "$HOME/bin/fx"

RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/noahgorstein/jqp/releases/download/v0.5.0/jqp_Linux_x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/arl/gitmux/releases/download/v0.10.3/gitmux_v0.10.3_linux_amd64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/rs/curlie/releases/download/v1.7.2/curlie_1.7.2_linux_amd64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/jesseduffield/lazydocker/releases/download/v0.23.1/lazydocker_0.23.1_Linux_x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
...
RUN TEMP_TAR_GZ="$(mktemp)" \
    TEMP_DIR="$(mktemp -d)" \
    && wget -q -O "$TEMP_TAR_GZ" 'https://github.com/jesseduffield/lazygit/releases/download/v0.40.2/lazygit_0.40.2_Linux_x86_64.tar.gz' \
    && tar -zxf "$TEMP_TAR_GZ" -C "$TEMP_DIR" \
