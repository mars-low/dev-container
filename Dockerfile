...
RUN TEMP_DEB="$(mktemp)" \
    && wget -nv -O "$TEMP_DEB" 'https://packages.microsoft.com/config/ubuntu/20.04/packages-microsoft-prod.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"
...
RUN TEMP_DEB="$(mktemp)" \
    && wget -nv -O "$TEMP_DEB" 'https://github.com/PowerShell/PowerShell/releases/download/v7.4.0/powershell_7.4.0-1.deb_amd64.deb' \
    && dpkg -i "$TEMP_DEB" \
    && rm -f "$TEMP_DEB"
